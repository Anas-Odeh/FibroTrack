...
class YOLOGUI:
    ...
    def process_images(self):
        ...
        without_bbox_folder = os.path.join(self.test_images_folder, "segmented_images_without_bbox")
        binary_masks_folder = os.path.join(self.test_images_folder, "binary_masks")
    
        os.makedirs(with_bbox_folder, exist_ok=True)
        os.makedirs(without_bbox_folder, exist_ok=True)
        ...
        for i, image_path in enumerate(image_paths):
            try:
                ...
                if len(new_result.boxes) > 0:
                    ...
                    max_conf_index = np.argmax(confidences)  # Index of the highest confidence detection
    
                    # Keep only the highest confidence detection
                    new_result.boxes = new_result.boxes[max_conf_index:max_conf_index + 1]
                    new_result.masks.data = new_result.masks.data[max_conf_index:max_conf_index + 1]
                ...
                result_image_with_bbox_pil.save(os.path.join(with_bbox_folder, f"segmented_{os.path.basename(image_path)}"))
    
                # Save instance-segmented image without bounding boxes
                if new_result.masks is not None:
                    masks = new_result.masks.data.cpu().numpy()
                    original_image_array = np.array(Image.open(image_path).convert("RGB"))
                    ...
                    for idx, mask in enumerate(masks):
                        ...
                        black_background[resized_mask] = original_image_array[resized_mask]
    
                        # Save binary mask
                        binary_mask = (resized_mask > 0.5).astype(np.uint8) * 255
                        binary_mask_image = Image.fromarray(binary_mask).convert("L")
                        binary_mask_image.save(os.path.join(binary_masks_folder, f"mask_{idx}_{os.path.basename(image_path)}"))
    
                        # Calculate region properties and save them
                        props = regionprops_table(binary_mask, properties=list(PROPS.values()))
                        for key, value in PROPS.items():
                            if key in props:
                                props[key] = props[value]
                        props_data = {
                            "Image Name": os.path.basename(image_path),
                            **{prop: props[prop][0] for prop in props.keys() if len(props[prop]) > 0}
                        }
                    ...
                    result_image_without_bbox_pil.save(os.path.join(without_bbox_folder, f"segmented_{os.path.basename(image_path)}"))
        ...
        if area_data:
            ...
            excel_path = os.path.join(self.test_images_folder, "Muscle_Region_Properties.xlsx")
